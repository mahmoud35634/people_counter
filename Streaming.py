...
def count_persons_on_image(image):
...
cap=cv2.VideoCapture('Live 20 _10 (online-video-cutter.com).mp4')   # 'vtest.avi'
...
#    frame = stream.read()
    ...
    bbox_id, num_persons = count_persons(frame)
...
#                 count_it = count_persons_on_image(result)
                _,count_it = count_persons(result)
                cv2.putText(result, 'Count: ', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 4)
                cv2.putText(result, str(count_it), (160, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 4)
